...
for (i in seq_along(problems)) {
  for (j in 1:1:nlyr(solutions)) {
    if (i == j) {
      ...
      scenarios_performance_models <- rbind(
        scenarios_performance_models,
...
for (i in seq_along(problems)) {
  for (j in 1:1:nlyr(solutions)) {
    if (i == j) {
      ...
      scenarios_performance_species <- rbind(
        scenarios_performance_species,
...
ggscatter(corr.test,
  x = "relative_held.1", y = "relative_held.3",
